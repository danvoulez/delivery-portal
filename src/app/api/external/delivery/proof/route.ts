...
import { AttachProofSchema } from '@/modules/deliveries-public-portal/http/schemas';

export async function POST(req: Request) {
  try {
    const { facade, issuer } = createDeliveriesPublicPortalFacade();
    const session = await readPortalSessionFromRequest(issuer);
    const body = await readValidatedJson(req, AttachProofSchema);
    await facade.attachProofFromSession(session, {
